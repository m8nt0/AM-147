...
function int = adapquad(f, a0, b0, tol0)
    ...
    while n > 0 % n is the current position at the end of the list
        ...
        if abs(oldapp - (app(n) + app(n+1))) < 3 * tol(n)
            ...
            tol(n) = tol(n) / 2;
            tol(n+1) = tol(n);
            n = n + 1; % go to the end of the list, repeat
        end
    end
end
...
% Exercise 2: Adaptive Quadrature
% Approximate the integral of the function (1 + sin(exp(3*x))) from -1 to 1.

% Function to integrate
f = @(x) 1 + sin(exp(3*x));

% Call the adaptive quadrature function
result = adapquad(f, -1, 1, 0.005);
